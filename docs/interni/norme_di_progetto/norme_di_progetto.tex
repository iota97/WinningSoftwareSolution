...
\item push su un branch personale;
\item eventuale merge su un branch collettivo;
\item pull request tramite piattaforma web GitHub per merge sul branch main;
\item review della pull request;
\item merge sul branch \texttt{main}.
\end{itemize}

Un branch collettivo è un branch su cui operano più membri del gruppo e ha come scopo l'elaborazione di un prodotto. Il branch si chiamerà come il prodotto.

Non sarà possibile fare commit su un branch collettivo ma ogni modifica avverrà attraverso il merge di un branch personale.

Ogni membro del gruppo può modifcare solamente i propri branch personali, che saranno nominati \texttt{$<$iniziali\_autore$>$-$<$prodotto$>$}. Dove \texttt{$<$prodotto$>$} indica il branch collettivo da cui deriva.
...
Dopo che è stato fatto il merge di un branch personale, quest'ultimo va eliminato.
...
\paragraph{Tracciamento dei progressi}
