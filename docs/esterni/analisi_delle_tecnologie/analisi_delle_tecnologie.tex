...
\subsection*{Backend}
Poiché l'architettura prevede necessariamente l'utilizzo di un server sono state valutate diverse opzioni per quanto riguarda la scelta del linguaggio e dell'eventuale framework per lo sviluppo della parte di backend. Tra questi sono stati presi in considerazione Java e Javascript poiché ci consentono di utilizzare rispettivamente le librerie Web3j e Web3js che permettono di emettere transazioni in blockchain nonchè gestire oggetti (transazione, contratto, ecc) comodamente. La scelta è infine ricaduta su Java poichè, oltre ad essere stato consigliato dal proponente, si tratta di un linguaggio nativamente ad oggetti ed è fortemente tipizzato, al contrario di JavaScript che costringe all'utilizzo di TypeScript per l'integrazione degli oggetti.
Per quanto riguarda invece la scelta del framework per lo sviluppo è stato scelto Java Spring: offre infatti un'ampia documentazione ed è inoltre il framework consigliato dal proponente.

\subsubsection{Pagamento automatizzato alla messa in vendita}
...
Tale script verrà sviluppato in Javascript per alta compatibilità (ogni Ecommerce avrà backend diversi) e per la possibilità di utilizzare la libreria Web3js per firmare ed emettere transazioni da parte dei venditori in modo automatico (accedendo direttamente al wallet senza la necessità di approvare manualmente le transazioni con Metamask).
Non utilizzando Metamask, il nodo presso la quale emettere le transazioni sarà fornito dal servizio di API per Polygon "Infura".

\subsubsection{Database}
Per la memorizzazione dei vari ID di ogni prodotto di ciascun venditore è necessario l'utilizzo di un database relazionale. Poiché non sono emerse necessità specifiche per le quali utilizzare un determinato database, è stato scelto PostgreSQL poiché suggerito dal proponente. 

\subsection*{Frontend}
Sarà necessaria la realizzazione lato frontend di una WebApp per permettere a venditori, Ecommerce e acquirenti di interagire in modo facile ed intuitivo per le attività di acquisto, monitoraggio dei vari ordini e infine per lo sblocco dei fondi alla ricezione del pacco con scansione del QR-code.

\subsubsection*{Acquisto e monitoraggio ordini}
Per quanto riguarda la realizzazione della WebApp per l'acquisto e il monitoraggio ordini è stato scelto il framework Javascript Angular. È stato scelto poiché rispetta le nostre esigenze ed è ottimo per la realizzazione di WebApp multipiattaforma. Javascript ci consentirà inoltre di utilizzare la sopracitata libreria Web3js per gestire le transazioni in blockchain nonchè interfacciarsi comodamente con l'estensione di Metamask per la firma delle transazioni.

\subsubsection*{Scansione QR code}
Per quanto riguarda la realizzazione della WebApp per la scansione del QR code al momento della ricezione del pacco è stato scelto il toolkit di Google Mobile Flutter. Oltre ad essere il framework consigliatoci dalla proponente, è stato scelto anche per la possibilità di realizzare app mobile per più sistemi operativi senza necessità di adattare e/o riscrivere il codice.
