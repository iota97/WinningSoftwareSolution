...
Tale script verrà sviluppato in Python per comodità di sviluppo e facilità di integrazione.
...
Per la memorizzazione dei dati di nostro interesse circa le transazioni è necessario l'utilizzo di un database. Poiché non sono emerse necessità specifiche per le quali utilizzare un determinato database piuttosto che un altro è stato scelto PostgreSQL poiché suggerito dalla Proponente.

\subsection*{Frontend}
Sarà necessaria la realizzazione di una WebApp per permettere a venditori, Ecommerce e acquirenti di interagire in modo facile ed intuitivo per le attività di acquisto, monitoraggio dei vari ordini e infine per lo sblocco dei fondi alla ricezione del pacco con scansione del QR-code.
