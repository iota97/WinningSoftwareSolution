...
describe('ServerManager', () => { 
    const serverMaker: ServerManager = new ServerManager();
    
    it('Server - Before', async () => {
        serverMaker.closeServer();
        process.env.PORT = String(Number(process.env.PORT as string) - 1)
    })
    it('Server - SQL', async () => {
        serverMaker.setSQL(new SQL_Mock);
    })
    it('Server - PageCreator', async () => {
        serverMaker.setPageCreator(new PageCreator);
    })
    it('Server - PageCreator', async () => {
        serverMaker.setContract(new Web3_Contract_Mock1);
    })
